### 👋 Hi, I’m @postname
#### 💼 Working as a Technical Consultant, currently focused on Network Administration.
#### 📖 My current study is Cloud & Security Engineering

## 👀 Current Interests
- **Cybersecurity:** OSINT, ethical hacking, digital forensics  
- **Cloud Automation:** Infrastructure as Code, Docker & Kubernetes, observability & monitoring  
- **Cloud Security Governance:** risk analysis & compliance, policy & governance, translating policy to operations

## 🔭 Current Skills & Tools
- **Programming:** C#, C++  
- **Web:** HTML, CSS, JavaScript  
- **Databases:** SQL  
- **Cloud & Infra:** Infrastructure as Code (Terraform / ARM / CloudFormation), Docker, Kubernetes  
- **Security:** OSINT methods, penetration testing basics, forensic fundamentals  
- **CI/CD & Automation:** pipelines, scripting, DevOps principles  
- **Networking:** CCNA certified — networking fundamentals, routing & switching

## 🛠️ Working Approach & Learning Goals
- Hands-on labs and practical projects  
- Justify technical choices to stakeholders with clear reports and rationale  
- Work using Agile and DevOps principles  
- Career goals: Security Analyst, Cloud/DevOps Engineer, or Forensic Investigator

<!---
postname/postname is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
